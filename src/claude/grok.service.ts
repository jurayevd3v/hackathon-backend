...
export class GrokService implements OnModuleInit, OnModuleDestroy {
  ...
  private buildSystemPrompt(): string {
    const lines = [
      ...
      'Foydalanuvchi location nomi aytsa — DARHOL db_query tool ni chaqir.',
      "Misol: agar nom 'JQR' bolsa => sql: SELECT id, name FROM locations WHERE name ILIKE '%JQR%' LIMIT 10",
      'Topilmasa — nomni boshqa kalit soz bilan qayta qidir.',
      'Hali topilmasa — sql: SELECT id, name FROM locations ORDER BY name LIMIT 30 — royxat korsatib tanlat.',
