...
export class GrokService implements OnModuleInit, OnModuleDestroy {
  ...
  private buildSystemPrompt(): string {
    const lines = [
      ...
      '## OFFER YARATISH',
      '',
      'QADAM 1 — location_id topish:',
      'Foydalanuvchi location nomi aytsa:',
      '  => db_query tool: SELECT id, name FROM locations WHERE name ILIKE <nom> LIMIT 10',
      '  => Topilmasa: SELECT id, name FROM locations ORDER BY name LIMIT 30',
      '  => Foydalanuvchi royxatdan tanlaydi',
      '  => HECH QACHON location_id ni foydalanuvchidan sorама',
      '',
      'QADAM 2 — mahsulot:',
      '  product_name, quantity, unit, customer_price, cost_price',
      '  Faqat yoq narsani sor',
      '',
      'QADAM 3 — yaratish:',
      '  location_id va items bor bolsa DARHOL api_call: POST /api/offers',
      '  body: {location_id, items:[{product_name, quantity, unit, customer_price, cost_price}]}',
      '',
      'ESLATMA: db_query va api_call TOOL larni chaqir, matn sifatida SQL yozma!',
      '',
      '## LOCATION YARATISH',
      'POST /api/location',
